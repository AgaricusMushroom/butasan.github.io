...
function volumeChange(){  //ＢＧＭとＳＥの有無を変更
	if(se == true){
		se = false;
		document.getElementById("typevolume").src = "bgmse2.png";
		localStorage.setItem("AgaricusSpeedOptionVolume", "false");
	} else {
		se = true;
		document.getElementById("typevolume").src = "bgmse1.png";
		localStorage.setItem("AgaricusSpeedOptionVolume", "true");
	}
}
...
function gameset(){  //タイトルに戻す処理
	...
	document.getElementById("savage3").style.display = "inline";
	...
	document.getElementById("typevolume").style.display = "inline";
...
function arrayset(dif){  //ゲームスタートさせる処理
	...
	case 1:  //Lv1
		...
		AILevel = 1;  //ＡＩの判断レベル。高いほど嫌らしい出し方をする
		break;
	case 2:  //Lv2
	...
	case 4:  //Lv4
		...
		AILevel = 2;  //ＡＩの判断レベル。高いほど嫌らしい出し方をする
		break;
	case 5:  //Lv5
	...
	default:  //Lv6
		move_speed = 20;  //相手のカード移動にかかる時間。10で0.1秒
		think_speed = 300;  //カード判断の周期　ドローは４分の３として計算
		AILevel = 3;  //ＡＩの判断レベル。高いほど嫌らしい出し方をする
	}
	for(i = 1; i < 27; i++){
		talon1[i] = -1;
		talon2[i] = -1;
	...
	document.getElementById("savage3").style.display = "none";
	document.getElementById("typelead").style.display = "none";
	document.getElementById("typevolume").style.display = "none";
...
function checkMyHand(testLeft,testRight){
	if(AILevel <= 2){
		ignore = true;
		return true;
	}
	if(ignore == true) return true;
	for(var testNumber = 1; testNumber < 5; testNumber++){
		if(handdisp1[testNumber] == testLeft - 1 || handdisp1[testNumber] == testLeft + 1){
			return false;
		}
		if(lead_KA && ((handdisp1[testNumber] == 1 && testLeft == 13) || (handdisp1[testNumber] == 13 && testLeft == 1))){
			return false;
		}
		if(handdisp1[testNumber] == testRight - 1 || handdisp1[testNumber] == testRight + 1){
			return false;
		}
		if(lead_KA && ((handdisp1[testNumber] == 1 && testRight == 13) || (handdisp1[testNumber] == 13 && testRight == 1))){
			return false;
		}
	}
	ignore = true;
...
function checkHand(test){
	if(AILevel <= 1){
		ignore2 = true;
		return true;
	}
	if(ignore2 == true) return true;
	for(var testNumber = 1; testNumber < 5; testNumber++){
		if(handdisp1[testNumber] == test - 1 || handdisp1[testNumber] == test + 1){
			checkOk = true;
			return true;
		}
		if(lead_KA && ((handdisp1[testNumber] == 1 && test == 13) || (handdisp1[testNumber] == 13 && test == 1))){
			checkOk = true;
			return true;
		}
	}
...
function think(){  //定期処理
	if(lastFlg == false && (talon1rem == 0 || talon2rem == 0)) playBgm(1);
	if(thinkstop == false){
		...
		for(a = 5; a < 9; a++){
			if((targetset != 2 && handdisp2[a] == left - 1) || (targetset != 2 && handdisp2[a] == left + 1)){
				if(checkHand(left)){
					if(checkMyHand(handdisp2[a],right)){ 
						targetset = 1;
						thinking += 10;
			...
			if((targetset != 1 && handdisp2[a] == right - 1) || (targetset != 1 && handdisp2[a] == right + 1)){
				if(checkHand(right)){
					if(checkMyHand(left,handdisp2[a])){ 
						targetset = 2;
						thinking += 10;
						if(thinking < think_speed) return;
						thinking = 0;
		...
		if(ignore2 == false){
			ignore2 = true;
			if(AILevel >= 2 && checkOk == false)thinking += 10;
			return;
		}
		if(ignore == false){
			ignore = true;
			if(AILevel >= 3)thinking += 10;
			return;
		}
		for(a = 5; a < 9; a++){
...
function move_set2(myDraw,enemyDraw){  //仕切り直しの座標設定
	...
	startX1 = parseFloat(deck1.offsetLeft);
	startY1 = parseFloat(deck1.offsetTop);
	startX2 = parseFloat(deck2.offsetLeft);
	startY2 = parseFloat(deck2.offsetTop);
	moveX1 = parseFloat(targetBA1.offsetLeft);
	moveY1 = parseFloat(targetBA1.offsetTop);
	moveX2 = parseFloat(targetBA2.offsetLeft);
	moveY2 = parseFloat(targetBA2.offsetTop);
...
function save(){  //戦歴保存の処理
	...
	case 6:
		lv6 = work;
		localStorage.setItem("AgaricusSpeedLv6", lv6);
		break;
	}
	l1 = lv1 % 1000000;
	l1 -= (lv1 % 1000);
...
function load() {  //ページ読み込み時の処理
   ...
   if(window.localStorage) {  //ローカルストレージに対応してる？
	...
	if(localStorage.getItem("AgaricusSpeedOptionVolume") == "false") volumeChange();
	lv1 = 0;
	...
	lv6 = 0;
	local=localStorage.getItem("AgaricusSpeedLv6");
	if(local != null) lv6 = local;
	l1 = lv1 % 1000000;
	l1 -= (lv1 % 1000);
	l1 = parseInt(l1 / 1000);
	l2 = lv2 % 1000000;
...
function playSound(no){
	...
	case 6:
		...
		break;
	case 7:
   		sound.src = "result.mp3";
		break;
	case 8:
		sound.src = "decision.mp3";
		break;
	}
	sound.play();
}
...
function playBgm(no){
	...
	case -1:
		...
		break;
	case 0:
   		audio.src = "03_Ruin_Rummage.ogg";
		break;
	case 1:
		lastFlg = true;
   		audio.src = "2000_Battle3.ogg";
		break;
	case 2:
		audio.src = "VictoryLoop2.ogg";
		break;
	}
	audio.loop = true;
	audio.play();
